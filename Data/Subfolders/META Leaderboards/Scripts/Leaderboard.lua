...
local UPDATE_ON_SCORE_CHANGED = Leaderboard:GetCustomProperty("UpdateOnScoreChanged")

local LEADERBOARD_TYPES = { "GLOBAL", "MONTHLY", "WEEKLY", "DAILY" }
local LEADERBOARD_STATS = { "SCORE" }
...
local function OnUpdate(leaderboardPersistence, leaderboardStat, player, score, resourceName)
	...
	elseif(leaderboardStat == "SCORE") then
		if(LEADERBOARD_STAT ~= "SCORE") then return end

		Leaderboards.SubmitPlayerScore(LeaderboardReference, player, score)

		if(UPDATE_ON_SCORE_CHANGED) then
			Update()
		end
	else
